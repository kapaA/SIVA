<?php
	...
	$LivingRoom = mysql_query("SELECT * FROM tempMeas  WHERE id = 1 LIMIT 1700, 10000");
	$MasterBedroom = mysql_query("SELECT * FROM tempMeas  WHERE id = 2 LIMIT 1700, 10000");
	$outRoom = mysql_query("SELECT * FROM tempMeas WHERE id = 3 LIMIT 2500, 10000");

	
	
	
	while($lRData=mysql_fetch_array($LivingRoom)){
		$lR[] = array($lRData['time'],(float)$lRData['hum']); 
	} 
	while($mBData=mysql_fetch_array($MasterBedroom)){
		$mB[] = array(($mBData['time']),(float)$mBData['hum']); 
	}
	while($outData=mysql_fetch_array($outRoom)){
		$out[] = array(($outData['time']),(float)$outData['hum']); 
	}
